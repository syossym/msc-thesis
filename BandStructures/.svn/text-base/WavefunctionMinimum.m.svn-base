function [a, E, error] = WavefunctionMinimum(Profile, Orientation, E_min, E_max, k_t, E_acc, E_step, min_num)
...
% Optionally, the function can look for the min_num-th minimum, disregarding
% the first mun_num-1 minima in the interval. 
...
if (no_sign_chg)    % <-- no minimum
    ...
    if (E_step > E_acc)
        ...
        [a,E,error] = WavefunctionMinimum(Profile,Orientation,E-2*E_step,E,k_t,E_acc); 
    else
        a = c_min(ii-1);
        E = E - E_step;
        error = 0;
