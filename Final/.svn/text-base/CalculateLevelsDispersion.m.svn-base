function [Bands, QWParams] = CalculateLevelsDispersion(Structure, Bands, Params)
%
...
%          'Bands' - calculated conduction and valence band parameters.
...
k_t_step = 5e6;                   % k_t step for E-k dispersion
k_t_vec = [0:k_t_step:k_t_max];
Bands.k_t_vec = k_t_vec(Params.k_indices);

%% Conduction band

for (cc=1:Params.num_cond_subbands)
    Bands.Cond.E_k(cc,:) = Bands.Cond.E_0(cc)*Consts.e_0 + (Consts.hbar^2*k_t_vec.^2)./(2*Structure.QuantumStruct.ActiveLayers{1}.m_e);
end
...
switch (Params.ValenceCalculationMethod)
    case 'PropagationMatrix',
        [Bands, QWParams] = PropagatingMatrixMethod(Structure, Bands, Params);
    case 'k.p',
        [Bands, QWParams] = KdotPMethod(Structure, Bands, Params);
end

function [Bands, QWParams] = PropagatingMatrixMethod(Structure, Bands, Params)
...
Decimated_Profile = DecimatePotentialProfile(L_clad, Structure.QuantumStruct, Bands);
Bands.z_grid = Decimated_Profile.z_grid;
Bands.Profile = Decimated_Profile;

k_t_mat = Bands.k_t_vec;
...
for (E = E_min:E_step:E_max)
    ...
    % Transfer matrix
    tf_mat = TransferMatrix(E, k_t, Decimated_Profile, Orientation);
    h_10(ii) = tf_mat(3,3);
    l_10(ii) = tf_mat(4,3);
    h_01(ii) = tf_mat(3,4);
    l_01(ii) = tf_mat(4,4);
    ...
    if (ii>1)
        ...
        if (ii>2)
            if (new_sign_d-old_sign_d==2)
                ...
                [a, E_result] = WavefunctionMinimum(Decimated_Profile,Orientation,E-2*E_step,E,k_t,E_acc);

                %disp(['E_result=' num2str(E_result/meV)]);

                if (nr==1) | ( (nr>1) & abs(E_result-E_state(nr-1))>min_E_diff )
                    E_state(nr) = E_result;
                    [f_h, f_l]  = PlotWavefunction(E_result,k_t,1,a,Decimated_Profile,Orientation);
    ...
    if (ii>1)
        ...
        if (ii>2)
            if (new_sign_d3-old_sign_d3==2)
                ...
                [a, E_result] = WavefunctionMinimum(Decimated_Profile,Orientation,E-2*E_step,E+2*E_step,k_t,E_acc,E_step/50);
                ...
                if (nr==1) | ( (nr>1) & abs(E_result-E_state(nr-1))>min_E_diff )
                    ...
                    if ( (f_l(N)^2 + f_h(N)^2)/(max(abs(f_l))^2+max(abs(f_h))^2) < 1e-4  )
                        ...
                        [a, E_result] = WavefunctionMinimum(Decimated_Profile,Orientation,E_result,E+2*E_step,k_t,E_acc,E_step/50,2);
                        E_state(nr) = E_result;
                        [f_h,f_l]   =  PlotWavefunction(E_result,k_t,1,a,Decimated_Profile,Orientation);
...
for (vb_num=1:Params.num_valence_subbands)
    Bands.Valence.Wf_0_Q{vb_num} = wf_h_mat{vb_num};
    for (cb_num=1:Params.num_cond_subbands)
        wf_c = Normalise_Wfs(Decimated_Profile.z_grid, interp1(Structure.QuantumStruct.z_grid, Bands.Cond.Wf_0(cb_num,:), Decimated_Profile.z_grid_partial, 'pchip'), 'Rel');
        Bands.Cond.Wf_0_Q(cb_num,:) = wf_c;
...
for (vv=1:Params.num_valence_subbands)
    ...
    while ((jj < size(k_t_mat,2)) && ~abort)
        ...
        [a,E_result,err] = WavefunctionMinimum(Decimated_Profile,Orientation,min_E,max_E,k_t,E_acc_act,E_step,min_num);
                ...
                if (jj<3)
                    ...
                    if (E_acc_act<E_result*eps*10)
                        if (~changed_num)
                            ...
                            [f_l, f_h] = PlotWavefunction(E_result, k_t, 1, a, Orientation);
    ...
    % Saving the results
    E_k_v{vv} = subband_E;
    Bands.Valence.E_k(vv,:) = interp1(k_t_mat, subband_E, Bands.k_t_vec, 'pchip');
    for (cc=1:Params.num_cond_subbands)
        QWParams.OptMatrixElement.TE(vv, cc, :) = interp1(k_t_mat, squeeze(rtsTE(vv,cc,:)), Bands.k_t_vec, 'pchip');
        QWParams.OptMatrixElement.TM(vv, cc, :) = interp1(k_t_mat, squeeze(rtsTM(vv,cc,:)), Bands.k_t_vec, 'pchip');
    end
...
function [Bands, QWParams] = KdotPMethod(Structure, Bands, Params)
...
%L_clad = 100e-10;    % [m]
%Profile = DecimatePotentialProfile(L_clad, Structure.QuantumStruct);
Profile = Structure.QuantumStruct;
...
function Profile = DecimatePotentialProfile(L_clad, Structure, Bands)

global Consts;

dz = Structure.z_grid(2)-Structure.z_grid(1);

L = 0;
for (ll=1:length(Structure.Layers))
    if (Structure.Layers{ll}.IsActive)
        z = L-L_clad:dz:L+Structure.Layers{ll}.L*1e-10+L_clad;
        z = z(1:end-1);
        Profile.v_h_profile = interp1(Structure.z_grid, -(Bands.Valence.Vh_profile-Bands.Valence.Phi), z, 'cubic').'*Consts.e_0;
        Profile.v_h_profile = Profile.v_h_profile + abs(min(Profile.v_h_profile));
        Profile.v_e_profile = interp1(Structure.z_grid, (Bands.Cond.Ve_profile-Bands.Cond.Phi), z, 'cubic').'*Consts.e_0;
        %Profile.v_e_profile = Profile.v_e_profile + abs(min(Profile.v_e_profile));
        Profile.gamma1_profile = interp1(Structure.z_grid, Structure.gamma1_profile, z, 'cubic').';
        Profile.gamma2_profile = interp1(Structure.z_grid, Structure.gamma2_profile, z, 'cubic').';
        Profile.gamma3_profile = interp1(Structure.z_grid, Structure.gamma3_profile, z, 'cubic').';
        Profile.z_grid_partial = z;
        Profile.z_grid = (0:dz:dz*(length(Profile.z_grid_partial)-1)).';
        break;
    end
    L = L + Structure.Layers{ll}.L*1e-10;
end