function PlotOpticalSimulationParams_Ver2(Structure, Bands, QWParams, MCParams, Params)
%
...
% FCT
% h_AntiCrossing_TE_FCT = figure('Name', 'Anticrossing Diagrams (TE)');
% h_AntiCrossing_TM_FCT = figure('Name', 'Anticrossing Diagrams (TM)');
% h_AntiCrossing_TE_TM_FCT = figure('Name', 'Anticrossing Diagrams (TE & TM)');
h_AntiCrossing_TE_Waterfall_FCT = figure('Name', 'Anticrossing Diagrams (TE)');
h_AntiCrossing_TM_Waterfall_FCT = figure('Name', 'Anticrossing Diagrams (TM)');
% h_AntiCrossing_Waterfall_FCT = figure('Name', 'Anticrossing Diagrams');
% h_Susceptability_TE_FCT = figure('Name', 'QW Susceptibility (TE)');
% h_Susceptability_TM_FCT = figure('Name', 'QW Susceptibility (TM)');
% h_n_TE_FCT = figure('Name', 'QW Refractive Index (TE)');
% h_n_TM_FCT = figure('Name', 'QW Refractive Index (TM)');
% h_Susceptability_TE_Waterfall_FCT = figure('Name', 'QW Refractive Index (TE)');
% h_Susceptability_TM_Waterfall_FCT = figure('Name', 'QW Refractive Index (TM)');

% HF
% h_AntiCrossing_TE_HF = figure('Name', 'Anticrossing Diagrams (TE)');
% h_AntiCrossing_TM_HF = figure('Name', 'Anticrossing Diagrams (TM)');
% h_AntiCrossing_TE_TM_HF = figure('Name', 'Anticrossing Diagrams (TE & TM)');
h_AntiCrossing_TE_Waterfall_HF = figure('Name', 'Anticrossing Diagrams (TE)');
h_AntiCrossing_TM_Waterfall_HF = figure('Name', 'Anticrossing Diagrams (TM)');
% h_AntiCrossing_Waterfall_HF = figure('Name', 'Anticrossing Diagrams');
% h_Susceptability_TE_HF = figure('Name', 'QW Susceptibility (TE)');
% h_Susceptability_TM_HF = figure('Name', 'QW Susceptibility (TM)');
% h_n_TE_HF = figure('Name', 'QW Refractive Index (TE)');
% h_n_TM_HF = figure('Name', 'QW Refractive Index (TM)');
% h_Susceptability_TE_Waterfall_HF = figure('Name', 'QW Refractive Index (TE)');
% h_Susceptability_TM_Waterfall_HF = figure('Name', 'QW Refractive Index (TM)');
...
switch (sim_type)
    ...
    case 1,    % multiple DEG concentrations and single gamma value
        ...
        for (ii=1:length(MCParams(:,1)))
            ...
            y_var.units = '';
            grid_range = [1.515, 1.55];
            
            arranged_curves_TE = ArrangeAntiCrossingData(MCParams{ii,1}.Detuning.FCT.E_min_MC,MCParams{ii,1}.Detuning.FCT.E_min_MCQW_TE);
            arranged_curves_TM = ArrangeAntiCrossingData(MCParams{ii,1}.Detuning.FCT.E_min_MC,MCParams{ii,1}.Detuning.FCT.E_min_MCQW_TM);
...
%             figure(h_AntiCrossing_TE_TM_FCT);
%             subplot(ceil(length(MCParams)/5), 5, ii); hold on; box on;
%             plot(arranged_curves_TM{1}.E_MC, arranged_curves_TM{1}.E_MC, 'r');
%             title(['N_{2DEG}=' strrep(num2str(Params.N_DEG_vec(ii),'%1.0e'), 'e+0', 'x10^{') '}cm^{-2}']);
%             axis([grid_range grid_range]);
                       
            %reflection_vector = 1:round(length(MCParams{ii,1}.Detuning.FCT.r_MCQW_TE(:,1))/reflection_resolution):length(MCParams{ii,1}.Detuning.FCT.r_MCQW_TE(:,1));
            reflection_vector = 1:reflection_resolution:length(MCParams{ii,1}.Detuning.FCT.r_MCQW_TE(:,1));
            
            figure(h_AntiCrossing_TE_Waterfall_FCT);
            h = subplot(ceil(length(MCParams)/5), 5, ii); hold on; box on;
            PlotWaterfall(MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0, abs(MCParams{ii,1}.Detuning.FCT.r_MCQW_TE(reflection_vector,:)), h, 'b', grid_range, y_var, 5);
            title(['N_{2DEG}=' strrep(num2str(Params.N_DEG_vec(ii),'%1.0e'), 'e+0', 'x10^{') '}cm^{-2}']);
            set(gca, 'YTickLabel', '');
            
            figure(h_AntiCrossing_TM_Waterfall_FCT);
            h = subplot(ceil(length(MCParams)/5), 5, ii); hold on; box on;
            PlotWaterfall(MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0, MCParams{ii,1}.Detuning.FCT.r_MCQW_TM(reflection_vector,:), h, 'b', grid_range, y_var, 5);
            title(['N_{2DEG}=' strrep(num2str(Params.N_DEG_vec(ii),'%1.0e'), 'e+0', 'x10^{') '}cm^{-2}']);
            set(gca, 'YTickLabel', '');
            
%             figure(h_AntiCrossing_Waterfall_FCT); hold on;
%             h = subplot(ceil(length(MCParams)/5), 5, ii); hold on; box on;
%             PlotWaterfall(MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0, MCParams{ii,1}.Detuning.FCT.r_MCQW_TE(reflection_vector,:), h, 'b', grid_range, y_var, 5);
%             PlotWaterfall(MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0, MCParams{ii,1}.Detuning.FCT.r_MCQW_TM(reflection_vector,:), h, 'r', grid_range, y_var, 5);
%             title(['N_{2DEG}=' strrep(num2str(Params.N_DEG_vec(ii),'%1.0e'), 'e+0', 'x10^{') '}cm^{-2}']);
%             set(gca, 'YTickLabel', '');
            
%             figure(h_n_TE_FCT);
%             subplot(length(MCParams(:,1)),1,ii);
%             [AX,H1,H2] = plotyy(MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0, smooth(real(MCParams{ii,1}.Detuning.FCT.n_QW_TE), 30), MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0, smooth(imag(MCParams{ii,1}.Detuning.FCT.n_QW_TE), 30));
%             %set(get(AX(2), 'Ylabel'), 'String', '\Im[n_{QW}]');
%             set(H1,'LineStyle','-', 'Color', 'b');
%             set(H2,'LineStyle','-', 'Color', 'r');
%             set(AX(1), 'XLim', [min(MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0), max(MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0)]); %, 'YTick', [3.5, 5]);
%             set(AX(2), 'XLim', [min(MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0), max(MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0)]); %, 'YTick', [0, 1]);
...
%             figure(h_n_TM_FCT);
%             subplot(length(MCParams(:,1)),1,ii);
%             [AX,H1,H2] = plotyy(MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0, smooth(real(MCParams{ii,1}.Detuning.FCT.n_QW_TM), 30), MCParams{ii,1}.Detuning.FCT.E_vec/Consts.e_0, smooth(imag(MCParams{ii,1}.Detuning.FCT.n_QW_TM), 30));
%             %set(get(AX(2), 'Ylabel'), 'String', '\Im[n_{QW}]');
...
%             end
            ...
            n_QW_TE_Waterfall(ii,:) = MCParams{ii,1}.Detuning.FCT.n_QW_TE;
            n_QW_TM_Waterfall(ii,:) = MCParams{ii,1}.Detuning.FCT.n_QW_TM;
                       
        end
        
%         figure(h_Susceptability_TE_FCT);
%         xlabel('E (eV)');
%         figure(h_Susceptability_TM_FCT);
%         xlabel('E (eV)');
...
%         PlotWaterfall(MCParams{1,1}.Detuning.FCT.E_vec/Consts.e_0, real(n_QW_TE_Waterfall), h_Susceptability_TE_Waterfall_FCT, 'b', grid_range, y_var);
%         PlotWaterfall(MCParams{1,1}.Detuning.FCT.E_vec/Consts.e_0, imag(n_QW_TE_Waterfall), h_Susceptability_TE_Waterfall_FCT, 'r', grid_range);
%         ylabel('Re[n_{QW}] (blue), Im[n_{QW}] (red) (a.u.)');
%         xlabel('E (eV)');
%         title(['Refractive Index (TE): T=' num2str(Params.T) 'K']);
%         set(gca, 'YTickLabel', '');
%         
%         PlotWaterfall(MCParams{1,1}.Detuning.FCT.E_vec/Consts.e_0, real(n_QW_TM_Waterfall), h_Susceptability_TM_Waterfall_FCT, 'b', grid_range, y_var);
%         PlotWaterfall(MCParams{1,1}.Detuning.FCT.E_vec/Consts.e_0, imag(n_QW_TM_Waterfall), h_Susceptability_TM_Waterfall_FCT, 'r', grid_range);
%         ylabel('Re[n_{QW}] (blue), Im[n_{QW}] (red) (a.u.)');
%         xlabel('E (eV)');
...
%         set(gca, 'YTickLabel', '');
        ...
        for (ii=1:length(MCParams(:,1)))
            ...
            arranged_curves_TE = ArrangeAntiCrossingData(MCParams{ii,1}.Detuning.HF.E_min_MC,MCParams{ii,1}.Detuning.HF.E_min_MCQW_TE);
            arranged_curves_TM = ArrangeAntiCrossingData(MCParams{ii,1}.Detuning.HF.E_min_MC,MCParams{ii,1}.Detuning.HF.E_min_MCQW_TM);
            
%             figure(h_AntiCrossing_TE_TM_HF);
%             subplot(ceil(length(MCParams)/5), 5, ii); hold on; box on;
%             plot(arranged_curves_TM{1}.E_MC, arranged_curves_TM{1}.E_MC, 'r');
%             title(['N_{2DEG}=' strrep(num2str(Params.N_DEG_vec(ii),'%1.0e'), 'e+0', 'x10^{') '}cm^{-2}']);
%             axis([grid_range grid_range]);
                   
            reflection_vector = 1:reflection_resolution:length(MCParams{ii,1}.Detuning.HF.r_MCQW_TE(:,1));
            
            figure(h_AntiCrossing_TE_Waterfall_HF);
            h = subplot(ceil(length(MCParams)/5), 5, ii); hold on; box on;
            PlotWaterfall(MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0, abs(MCParams{ii,1}.Detuning.HF.r_MCQW_TE(reflection_vector,:)), h, 'b', grid_range, y_var, 5);
            title(['N_{2DEG}=' strrep(num2str(Params.N_DEG_vec(ii),'%1.0e'), 'e+0', 'x10^{') '}cm^{-2}']);
            set(gca, 'YTickLabel', '');
            
            figure(h_AntiCrossing_TM_Waterfall_HF);
            h = subplot(ceil(length(MCParams)/5), 5, ii); hold on; box on;
            PlotWaterfall(MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0, MCParams{ii,1}.Detuning.HF.r_MCQW_TM(reflection_vector,:), h, 'b', grid_range, y_var, 5);
            title(['N_{2DEG}=' strrep(num2str(Params.N_DEG_vec(ii),'%1.0e'), 'e+0', 'x10^{') '}cm^{-2}']);
            set(gca, 'YTickLabel', '');
            
%             figure(h_AntiCrossing_Waterfall_HF); hold on;
%             h = subplot(ceil(length(MCParams)/5), 5, ii); hold on; box on;
%             PlotWaterfall(MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0, MCParams{ii,1}.Detuning.HF.r_MCQW_TE(reflection_vector,:), h, 'b', grid_range, y_var, 5);
%             PlotWaterfall(MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0, MCParams{ii,1}.Detuning.HF.r_MCQW_TM(reflection_vector,:), h, 'r', grid_range, y_var, 5);
%             title(['N_{2DEG}=' strrep(num2str(Params.N_DEG_vec(ii),'%1.0e'), 'e+0', 'x10^{') '}cm^{-2}']);
%             set(gca, 'YTickLabel', '');
%             
%             figure(h_n_TE_HF);
%             subplot(length(MCParams(:,1)),1,ii);
%             [AX,H1,H2] = plotyy(MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0, smooth(real(MCParams{ii,1}.Detuning.HF.n_QW_TE), 30), MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0, smooth(imag(MCParams{ii,1}.Detuning.HF.n_QW_TE), 30));
%             %set(get(AX(2), 'Ylabel'), 'String', '\Im[n_{QW}]');
%             set(H1,'LineStyle','-', 'Color', 'b');
%             set(H2,'LineStyle','-', 'Color', 'r');
%             set(AX(1), 'XLim', [min(MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0), max(MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0)]); %, 'YTick', [3.5, 5]);
%             set(AX(2), 'XLim', [min(MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0), max(MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0)]); %, 'YTick', [0, 1]);
%             %set(AX(1), 'YLim', [3.5, max(real(MCParams{1,1}.Detuning.n_QW_TE))]);
%             %set(AX(2), 'YLim', [0, max(imag(MCParams{1,1}.Detuning.n_QW_TE))]);
%             set(AX(1), 'YColor', 'b');
%             set(AX(2), 'YColor', 'r');
...
%             figure(h_n_TM_HF);
%             subplot(length(MCParams(:,1)),1,ii);
%             [AX,H1,H2] = plotyy(MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0, smooth(real(MCParams{ii,1}.Detuning.HF.n_QW_TM), 30), MCParams{ii,1}.Detuning.HF.E_vec/Consts.e_0, smooth(imag(MCParams{ii,1}.Detuning.HF.n_QW_TM), 30));
%             %set(get(AX(2), 'Ylabel'), 'String', '\Im[n_{QW}]');
%             axis auto; axis tight;
%             set(H1,'LineStyle','-', 'Color', 'b');
%             set(H2,'LineStyle','-', 'Color', 'r');
...
%             end
            ...
            n_QW_TE_Waterfall(ii,:) = MCParams{ii,1}.Detuning.HF.n_QW_TE;
            n_QW_TM_Waterfall(ii,:) = MCParams{ii,1}.Detuning.HF.n_QW_TM;
                       
        end
        
%         figure(h_Susceptability_TE_HF);
%         xlabel('E (eV)');
%         figure(h_Susceptability_TM_HF);
%         xlabel('E (eV)');
...
%         PlotWaterfall(MCParams{1,1}.Detuning.HF.E_vec/Consts.e_0, real(n_QW_TE_Waterfall), h_Susceptability_TE_Waterfall_HF, 'b', grid_range, y_var);
%         PlotWaterfall(MCParams{1,1}.Detuning.HF.E_vec/Consts.e_0, imag(n_QW_TE_Waterfall), h_Susceptability_TE_Waterfall_HF, 'r', grid_range);
%         ylabel('Re[n_{QW}] (blue), Im[n_{QW}] (red) (a.u.)');
%         xlabel('E (eV)');
%         title(['Refractive Index (TE): T=' num2str(Params.T) 'K']);
%         set(gca, 'YTickLabel', '');
%         
%         PlotWaterfall(MCParams{1,1}.Detuning.HF.E_vec/Consts.e_0, real(n_QW_TM_Waterfall), h_Susceptability_TM_Waterfall_HF, 'b', grid_range, y_var);
%         PlotWaterfall(MCParams{1,1}.Detuning.HF.E_vec/Consts.e_0, imag(n_QW_TM_Waterfall), h_Susceptability_TM_Waterfall_HF, 'r', grid_range);
%         ylabel('Re[n_{QW}] (blue), Im[n_{QW}] (red) (a.u.)');
%         xlabel('E (eV)');
%         title(['Refractive Index (TM): T=' num2str(Params.T) 'K']);
%         set(gca, 'YTickLabel', '');
