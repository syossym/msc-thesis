...
%-----------------------------------------------------------------------------
% rand_ps function      Generate a random realization of a time series
...
if (prod(size(PowSpec))==2),
   ...
   FourComp = RandAmp.*Rand(:,1) + i.*RandAmp.*Rand(:,2);
   % negative frequencies f(-w) = f*(w) = conj(f(w))

   % inverse fourier transform (not evenly spaced)
   Value = zeros(Nt,1);
   for I=1:1:Nt,
      Value(I) = sum(FourComp.*exp(+2.*pi.*i.*Time(I).*FreqVec)) + ...,
                 sum(conj(FourComp).*exp(-2.*pi.*i.*Time(I).*FreqVec));
   end
