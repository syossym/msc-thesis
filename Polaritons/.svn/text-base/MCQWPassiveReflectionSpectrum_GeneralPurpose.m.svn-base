...
E_vec = [1.4:0.000025:1.65]*Consts.e_0;
...
for (ss=1:length(Structure))
    ...
    params.E = 0; %(E_vec(1)+0.5*(E_vec(end)-E_vec(1)))/Consts.e_0;
...
% Reflection spectrum
h_waitbar = waitbar(0,'Calculating Spectrum...');
for (gg=1:length(lambda_vec))
    waitbar(gg/length(lambda_vec),h_waitbar);
