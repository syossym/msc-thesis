function FullStructureParams = CalculateFullStructureReflection(EStruct, QStruct, Valence, Cond, OpticalParams, params)
...
FullStructureParams = [];
E_vec = [1.52:0.00005:1.56]*Consts.e_0;
...
for (ss=1:length(EStruct))
    waitbar(ss/length(EStruct),h);
    temp_params.x = EStruct{ss}.x;
    temp_params.E = 1.525; %(E_vec(1)+0.5*(E_vec(end)-E_vec(1)))/Consts.e_0;
    temp_params.T = params.T;
    mat = GetMaterial(EStruct{ss}.Name, temp_params);
    n_vec_profile(ss) = mat.n;
    n_vec_calc_init(ss,:) = GetRefractiveIndex(EStruct{ss}.Name, E_vec./Consts.e_0);
    l_vec(ss) = delta*EStruct{ss}.L*1e-10;      % [m]
end
close(h);
...
% 
% r = zeros(length(QStruct.N_DEG_vec), length(E_vec));
% for (con_num=1:length(OpticalParams))
%     Xi_e_Re = interp1(params.E_exc, smooth(real(OpticalParams{con_num}.Xi_TE_HF_vec)), E_vec, 'pchip');
%     Xi_e_Im = interp1(params.E_exc, smooth(imag(OpticalParams{con_num}.Xi_TE_HF_vec)), E_vec, 'pchip');
%     Xi_QW = Xi_QW + Xi_e_Re + 1i*Xi_e_Im;
%     n_QW(con_num, :) = sqrt(1+4*pi*Xi_QW);
%     n_vec_calc = n_vec_calc_init;
%     
%     for (ss=1:length(EStruct))
%         if (EStruct{ss}.IsActive)
%             n_vec_calc(ss,:) = n_QW(con_num, :);
%         end
%     end
%     
%     h = waitbar(0,['Calculating MCQW Spectrum, N_{DEG}=' num2str(QStruct.N_DEG_vec(con_num),'%1.0e') 'cm^{-2}']);
%     for (gg=1:length(lambda_vec))
%         waitbar(gg/length(lambda_vec),h);
...
%     text(E_vec(min_i)/Consts.e_0, min_v, [strrep(num2str(QStruct.N_DEG_vec(con_num),'%1.0e'), 'e+0', 'x10^{') '}'], 'FontSize', 7);
% end
% title('(b)');
% xlabel('E [eV]');
...
%     
%     subplot(length(OpticalParams),1,con_num);
...
%     end
%     con_text = strrep([num2str(QStruct.N_DEG_vec(con_num), '%1.0e')], 'e+0', 'x10^{');
%     con_text = [con_text '}'];
%     ylabel(con_text);
%     h_curr = gca;
%     if (con_num==1)
%         [min_MC,min_MC_i] = min(abs(r_MC));
%         title_text = ['|r_{MC}|^2 (red), |r_{MCQW}|^2 (blue): E_{MC}=' num2str(E_vec(min_MC_i)/Consts.e_0) 'eV, T=' num2str(params.T) 'K, \delta=' num2str(delta)];
%         title(title_text);
%     end
%     if (con_num~=length(OpticalParams))
...
%     h_curr = gca;
%     if (con_num==1)
%         title_text = ['\Re[n_{QW}] (blue), \Im[n_{QW}] (red): T=' num2str(params.T) 'K, \delta=' num2str(delta)];
%         title(title_text);
%     end
%     if (con_num~=length(OpticalParams))
%         set(AX(1), 'XTickLabel', '');
...
con_num = params.N_DEG_index;
num_mins = params.num_mins; 
if (length(QStruct.N_DEG_vec) > 1)
    N_DEG = QStruct.N_DEG_vec(con_num);
else
    N_DEG = QStruct.N_DEG_vec;
end
...
QW_Mat = GetMaterial('GaAs', temp_params);
Xi_QW = (QW_Mat.n.^2-1)/(4*pi);
Xi_e_Re = interp1(params.E_exc, (real(OpticalParams{con_num}.Xi_TE_HF_vec)), E_vec, 'pchip');
Xi_e_Im = interp1(params.E_exc, (imag(OpticalParams{con_num}.Xi_TE_HF_vec)), E_vec, 'pchip');
Xi_QW = Xi_QW + Xi_e_Re + 1i*Xi_e_Im;
n_vec_calc = n_vec_calc_init;
...
% title(['Level anticrossing diagram: $$T=' num2str(params.T) 'K$$, $$N_{2DEG}=' strrep([num2str(QStruct.N_DEG_vec(con_num), '%1.0e')], 'e+0', '\times 10^{') '}cm^{-2}$$'],'interpreter','latex');
% axis tight; set(gca, 'FontName', 'Times New Roman', 'FontSize', 8);

%% Saving the results

FullStructureParams.E_min_r_MC = E_min_r_MC;
FullStructureParams.E_min_r_MCQW = E_min_r_MCQW;
FullStructureParams.z_grid_calc = z_grid_calc;
FullStructureParams.n_profile = n_profile;
