    ...
    for (ee = 1:length(E_exc))
        ...
        for (cb_num = 1:cb_len)
            for (vb_num = 1:vb_len)
                ...
                M_TE = eye(k_len);
                M_TE = M_TE - repmat((dk./mu_TE).', 1, k_len).*repmat(Xi_0_TE.*k_vec, k_len, 1).*Theta{cb_num,vb_num};%(:,end:-1:1).*1e8;
                M_TM = eye(k_len);
                M_TM = M_TM - repmat((dk./mu_TM).', 1, k_len).*repmat(Xi_0_TM.*k_vec, k_len, 1).*Theta{cb_num,vb_num};%(:,end:-1:1).*1e8;
        ...
        G_TE(ee) = abs(C_g.*1i*w.*sum_TE*L_z);
        G_TM(ee) = abs(C_g.*1i*w.*sum_TM*L_z);
