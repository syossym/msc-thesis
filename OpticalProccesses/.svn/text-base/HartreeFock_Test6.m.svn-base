...
for (con_num = 1:length(con_vec))
    ...
    % Quasi Fermi levels
    options = optimset('TolFun',1e-100,'TolX',1e-100,'MaxIter',5000,'MaxFunEvals',5000,'Display','off');
    ...
    for (ee = 1:length(E_exc))
        ...
        for (cb_num = 1:cb_len)
            for (vb_num = 1:vb_len)
                ...
                M_TE = eye(k_len);
                M_TE = M_TE - repmat((dk./mu_TE).', 1, k_len).*repmat(Xi_0_TE, k_len, 1).*Theta{cb_num,vb_num};
                M_TM = eye(k_len);
                M_TM = M_TM - repmat((dk./mu_TM).', 1, k_len).*repmat(Xi_0_TM, k_len, 1).*Theta{cb_num,vb_num};
