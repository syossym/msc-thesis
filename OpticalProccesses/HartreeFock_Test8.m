...
con_vec = [2e10,6e10,2e11,6e11,2e12,6e12];    % [cm^-2]
gamma = 1e13;                        % [s^-1]
...
for (con_num = 1:length(con_vec))
   ...
   % Carrier concentrations
    ...
    disp(['-- Concentration - N=' num2str(N) 'cm^-2, vb=' num2str(P) 'cm^-2']);
    ...
    E_f = 3.6e-11*N*1e-3;     % eV
    E_g = GetMaterialBandGap(Res2.Materials{2}, T); % eV
    E_f = E_f + (Res2.E_k_c{1}(1)/Consts.e_0);
    ...
    plot(E_exc/Consts.e_0, G_TE_MBT, 'b', E_exc/Consts.e_0, G_TE_FCT, 'r'); hold on;
    ...
    plot(E_exc/Consts.e_0, G_TM_MBT, 'b', E_exc/Consts.e_0, G_TM_FCT, 'r'); hold on;
    ...
    subplot(313);
    plot(E_exc/Consts.e_0, R_sp_MBT, 'b', E_exc/Consts.e_0, R_sp_FCT, 'r'); hold on;
    %plot(E_exc/Consts.e_0, R_sp_MBT, colors(con_num), E_exc/Consts.e_0, R_sp_FCT, strcat(colors(con_num),':')); hold on;
    plot(deltaE_vc_0*ones(size(R_sp_MBT)), R_sp_MBT, 'g:', E_f*ones(size(R_sp_MBT)), R_sp_MBT, 'y', ...
         E_g*ones(size(R_sp_MBT)), R_sp_MBT, 'c', (Res2.E_k_c{1}(1)/Consts.e_0)*ones(size(R_sp_MBT)), R_sp_MBT, 'k', ...
         (Res2.E_k_c{2}(1)/Consts.e_0)*ones(size(R_sp_MBT)), R_sp_MBT, 'm');
